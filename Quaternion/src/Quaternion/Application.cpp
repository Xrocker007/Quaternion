...
namespace QUA {
	...
	void Application::Run()
	{
		...
		while (m_Window->IsWindowRunning())
		{
			...
			m_Renderer->drawRect(Vec2{ m_Physics->getPosition(body).x, m_Physics->getPosition(body).y }, Vec2{ bodySize.x, bodySize.y },
								Vec2{ 0 , 0 }, m_Physics->getAngle(body), { 1, 0.1, 0.1, 1 });
